...
모델 ID `portable_e2e.perspective_trajectory.physical_no_accel.v1`는 기존 physical-v1과 같은 구조에서 **입력 가속도 값의 영향만 제외**하는 비교 실험용입니다. 아직 이 ID로 실제 데이터 학습·주행·성능 검증을 진행하지 않았습니다. 가속도 기준점 문제를 물리적으로 고친 모델이나 자율주행 승인 모델이 아닙니다.
...
따라서 이 모델은 가속도 값 대신 속도 history·yaw rate 등 나머지 정보를 사용하게 됩니다. 이것이 정확도를 높일지 낮출지는 실제 비교가 필요합니다. 원래 데이터의 `ax/ay`나 XY/속도 정답을 수정하거나, 새 물리 가속도를 계산한 것이 아닙니다.
...
먼저 수집 데이터의 경로·속도·정지 품질을 통과시킵니다. 그 이후 동일한 새 corpus에서 unchanged physical-v1 대 새 no-accel ID를 같은 3개 seed·optimizer budget·loss·gate로 비교하는 별도 계획을 사전에 고정해야 합니다. 테스트셋으로 설정을 고르거나 자동으로 모델을 승격하지 않습니다. 이 문서는 학습 실행이나 gate 완화의 승인이 아닙니다.
