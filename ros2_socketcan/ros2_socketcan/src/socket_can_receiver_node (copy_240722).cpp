...
LNI::CallbackReturn SocketCanReceiverNode::on_configure(const lc::State & state)
{
  ...
  current_steering_tire_angle_topic_ = this->create_publisher<std_msgs::msg::Float64>("current_steering_rad_status", 500);
  ...
  //HH_230309
  vehicle_status_steering_status_topic_ = this->create_publisher<autoware_auto_vehicle_msgs::msg::SteeringReport>("/vehicle/status/steering_status", 500);
  //HH_230314
  vehicle_status_velocity_status_topic_ = this->create_publisher<autoware_auto_vehicle_msgs::msg::VelocityReport>("/vehicle/status/velocity_status", 500);
  // HH_240409 for Autoware Pr 532 
  vehicle_status_gear_status_topic_ = this->create_publisher<autoware_auto_vehicle_msgs::msg::GearReport>("/vehicle/status/gear_status", 500);
  vehicle_status_control_mode_topic_ = this->create_publisher<autoware_auto_vehicle_msgs::msg::ControlModeReport>("/vehicle/status/control_mode", 500);
  vehicle_status_hazard_lights_status_topic_ = this->create_publisher<autoware_auto_vehicle_msgs::msg::HazardLightsReport>("/vehicle/status/hazard_lights_status", 500);
  vehicle_status_turn_indicators_status_topic_ = this->create_publisher<autoware_auto_vehicle_msgs::msg::TurnIndicatorsReport>("/vehicle/status/turn_indicators_status", 500);

  //HH_230804 / for spring cloud
  // Gear_state
  // vehicle_status_gear_status_topic_=  this->create_publisher<std_msgs::msg::UInt8>("current_gear_state", 500);
  // Vehicle_type : 0(g-vehicle) 1(c-vehicle) 2(UGV)
  vehicle_type_topic_ = this->create_publisher<std_msgs::msg::UInt8>("vehicle_type", 500);
...
LNI::CallbackReturn SocketCanReceiverNode::on_activate(const lc::State & state)
{
  ...
  //HH_230704
  current_steering_tire_angle_topic_->on_activate();
  // HH_240409
  ...
  //HH_230804
  vehicle_status_gear_status_topic_->on_activate();
  vehicle_type_topic_->on_activate();
...
LNI::CallbackReturn SocketCanReceiverNode::on_deactivate(const lc::State & state)
{
  ...
  //HH_230704
  current_steering_tire_angle_topic_->on_deactivate();
  // HH_240409
  ...
  //HH_230804
  vehicle_status_gear_status_topic_->on_deactivate();
  vehicle_type_topic_->on_deactivate();
...
LNI::CallbackReturn SocketCanReceiverNode::on_cleanup(const lc::State & state)
{
  ...
  //HH_230704
  current_steering_tire_angle_topic_.reset();
  // HH_240409
  ...
  if (!enable_fd_) {
    ...
    while (rclcpp::ok()) {
      ...
      switch(frame_msg.id)
      {
        ...
        { 
          auto current_brake_press_report = frame_msg.data[0]/1.6;     //HH_221108 // full brake -> decimal :160, divide 1.6
          
          std_msgs::msg::Float64 current_brake_press_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
          if (current_brake_press_report >= 100) current_brake_press_report = 100;     //HH_221108
          // current_brake_press_msg.header.stamp = this->now(); // HH_240326 // no member header
        ...
        case 882: //0x372 //for gear state
        {
          ...
          u_int8_t gear_state_data = 0;

          if (gear_state_raw_data == 0x00) { gear_state_data = 22;} //P
          else if (gear_state_raw_data == 0x77) { gear_state_data = 20;} //R
          else if (gear_state_raw_data == 0x06) { gear_state_data = 1;} //N
          else if ( (gear_state_raw_data == 0x15) || (gear_state_raw_data == 0x25) ) { gear_state_data = 2;} //D
          
          // std_msgs::msg::UInt8 vehicle_status_gear_status_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
          // // vehicle_status_gear_status_msg.header.stamp = this->now(); // HH_240326 // no member header
          // vehicle_status_gear_status_msg.data = gear_state_data;
          // vehicle_status_gear_status_topic_->publish(std::move(vehicle_status_gear_status_msg)); 

          // HH_240409 
          autoware_auto_vehicle_msgs::msg::GearReport vehicle_status_gear_status_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
          vehicle_status_gear_status_msg.stamp = this->now();
          vehicle_status_gear_status_msg.report = gear_state_data;
          vehicle_status_gear_status_topic_->publish(std::move(vehicle_status_gear_status_msg)); 

          // vehicle_type
          std_msgs::msg::UInt8 vehicle_type_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
          // vehicle_type_msg.header.stamp = this->now(); // HH_240326 // no member header
          vehicle_type_msg.data = 0;
          vehicle_type_topic_-> publish(std::move(vehicle_type_msg)); 
        ...
        { 
          ...
          //HH_230309
          double current_steering_tire_angle_status_report = 0;
          ...
          current_EPS_En_ = (frame_msg.data[0] & 0X01); // EPS_En_status // 
          //std::cout << "EPS_En_: " << current_EPS_En_ << std::endl; // cruise on -> 1  //HH_221208 //HH_230914
          ...
          // current_steering_angle_msg.header.stamp = this->now(); // HH_240326 // no member header
          ...
          //HH_230531 for steering_tire_angle 
          current_steering_tire_angle_status_report = current_steering_angle_report * DEG2RAD / 16.16667; //HH_230615 //steering angle(rad)  --> steer to tire
          autoware_auto_vehicle_msgs::msg::SteeringReport vehicle_status_steering_status_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
          vehicle_status_steering_status_msg.stamp = this->now(); // HH_240409
          vehicle_status_steering_status_msg.steering_tire_angle = (current_steering_tire_angle_status_report); //HH_230314 // rad
          vehicle_status_steering_status_topic_->publish(std::move(vehicle_status_steering_status_msg));

          // //HH_230309
          // current_steering_tire_angle_status_report = current_steering_angle_report;
          // autoware_auto_vehicle_msgs::msg::SteeringReport vehicle_status_steering_status_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
          // vehicle_status_steering_status_msg.steering_tire_angle = (current_steering_tire_angle_status_report); //HH_230314 // degree
          // vehicle_status_steering_status_topic_->publish(std::move(vehicle_status_steering_status_msg));

          std_msgs::msg::Bool current_EPS_En_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
          // current_EPS_En_msg.header.stamp = this->now(); // HH_240326 // no member header
          ...
          u_int8_t vehicle_status_control_mode_report = 4;

          if ((current_cruise_status == 1) && (current_cruise_mode == false))
          {
            current_cruise_mode = true;              
            // HH_240409
            vehicle_status_control_mode_report = 1;

            std_msgs::msg::Bool current_cruise_mode_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
            // current_cruise_mode_msg.header.stamp = this->now(); // HH_240326 // no member header
            current_cruise_mode_msg.data = current_cruise_mode;
            current_cruise_mode_topic_->publish(std::move(current_cruise_mode_msg));

            // HH_240409
            autoware_auto_vehicle_msgs::msg::ControlModeReport vehicle_status_control_mode_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
            vehicle_status_control_mode_msg.stamp = this->now(); 
            vehicle_status_control_mode_msg.mode = (vehicle_status_control_mode_report);
            vehicle_status_control_mode_topic_->publish(std::move(vehicle_status_control_mode_msg));
          }
          else if ((current_cruise_status == 0))
          { 
            current_cruise_mode = false;
            // HH_240409
            vehicle_status_control_mode_report = 4;

            std_msgs::msg::Bool current_cruise_mode_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
            // current_cruise_mode.header.stamp = this->now(); // HH_240326 // no member header
            current_cruise_mode_msg.data = current_cruise_mode;
            current_cruise_mode_topic_->publish(std::move(current_cruise_mode_msg));  
        ...
        {           
          ...
          //HH_230816
          autoware_auto_vehicle_msgs::msg::VelocityReport vehicle_status_velocity_status_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
          vehicle_status_velocity_status_msg.longitudinal_velocity = (current_velocity_report / 3.6);
          vehicle_status_velocity_status_msg.header.frame_id = "base_link";  //HH_230316
          vehicle_status_velocity_status_topic_->publish(std::move(vehicle_status_velocity_status_msg));
          */
          ...
          // current_ACC_En_msg.header.stamp = this->now(); // HH_240326 // no member header
          ...
          // autoware_auto_vehicle_msgs::msg::VelocityReport vehicle_status_velocity_status_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
          // vehicle_status_velocity_status_msg.longitudinal_velocity = (current_velocity_status_report / 3.6);
          // vehicle_status_velocity_status_msg.header.frame_id = "base_link";  //HH_230316
          // vehicle_status_velocity_status_topic_->publish(std::move(vehicle_status_velocity_status_msg));
        ...
        case 1810: // 0x712
        {
          //HH_230927 ### The speed is not accurate. Replace with wheel speed
          int current_velocity_tmp = 0;
          int current_velocity_report = 0; 

          current_velocity_tmp = frame_msg.data[3]& 0xff;
          current_velocity_tmp = current_velocity_tmp << 8;
          current_velocity_tmp = frame_msg.data[2]| current_velocity_tmp;
          
          current_velocity_report = current_velocity_tmp * 0.03125;

          std_msgs::msg::Float64 current_velocity_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
          // current_velocity_msg.header.stamp = this->now(); // HH_240326 // no member header
          current_velocity_msg.data = current_velocity_report;
          current_velocity_topic_->publish(std::move(current_velocity_msg));
          // std::cout << "current_velocity_report: " << current_velocity_report << std::endl; 
          ...
          //HH_230816
          autoware_auto_vehicle_msgs::msg::VelocityReport vehicle_status_velocity_status_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
          vehicle_status_velocity_status_msg.longitudinal_velocity = current_velocity_report / 3.6; // mps to kph
          vehicle_status_velocity_status_msg.header.frame_id = "base_link";  //HH_230316
          vehicle_status_velocity_status_msg.header.stamp = this->now(); // HH_240326 
          vehicle_status_velocity_status_topic_->publish(std::move(vehicle_status_velocity_status_msg));

          // TODO:: 4 wheel vel avgerage  --> velocity
        }

        // HH_240722
        case 1811: // 0x713
        {
          // Extract the lateral acceleration from byte positions 0 and 1
          int16_t lateral_accel_raw = (msg->data[1] << 8) | msg->data[0];
          
          // Apply conversion rule
          double lateral_accel = 0.01 * (lateral_accel_raw - 1023);

          // Assuming a simple integration over a fixed time step to get lateral velocity
          double dt = 0.001; // Time step in seconds (example value)
          lateral_velocity_ += lateral_accel * dt;

          // Publish the lateral velocity
          autoware_auto_vehicle_msgs::msg::VelocityReport lateral_velocity_report_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);;
          lateral_velocity_report_msg.header.stamp = this->now();
          lateral_velocity_report_msg.longitudinal_velocity = 0.0; // Not used in this example
          lateral_velocity_report_msg.lateral_velocity = lateral_velocity_;
          lateral_velocity_report_msg.heading_rate = 0.0; // Not used in this example

          vehicle_status_lateral_accel_topic_->publish(velocity_report_msg);

          ////////////////////////////////////////////////////////////////
          // heading_rate | yaw rate
          int16_t heading_rate_raw = (msg->data[4] << 8) | msg->data[3];
          
          // Apply conversion rule
          double heading_rate = 0.01 * (heading_rate_raw - 4095);

          // Publish the lateral velocity
          autoware_auto_vehicle_msgs::msg::VelocityReport heading_rate_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);;
          heading_rate_msg.header.stamp = this->now();
          heading_rate_msg.longitudinal_velocity = 0.0; // Not used in this example
          heading_rate_msg.lateral_velocity = lateral_velocity_;
          heading_rate_msg.heading_rate = 0.0; // Not used in this example
