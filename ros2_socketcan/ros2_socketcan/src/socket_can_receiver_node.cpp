...
LNI::CallbackReturn SocketCanReceiverNode::on_configure(const lc::State & state)
{
  ...
  // Gear_state
  current_gear_state_topic_=  this->create_publisher<std_msgs::msg::UInt8>("current_gear_state", 500);
...
LNI::CallbackReturn SocketCanReceiverNode::on_activate(const lc::State & state)
{
  ...
  //HH_230804
  current_gear_state_topic_->on_activate();
...
LNI::CallbackReturn SocketCanReceiverNode::on_deactivate(const lc::State & state)
{
  ...
  //HH_230804
  current_gear_state_topic_->on_deactivate();
...
LNI::CallbackReturn SocketCanReceiverNode::on_cleanup(const lc::State & state)
{
  ...
  //HH_230804
  current_gear_state_topic_.reset();
    ...
    while (rclcpp::ok()) {
      ...
      switch(frame_msg.id)
      {
        ...
        case 882: //0x372 //for gear state
        {
          ...
          if (gear_state_raw_data == 0x00) { gear_state_data = 0;}
          else if (gear_state_raw_data == 0x77) { gear_state_data = 1;}
          else if (gear_state_raw_data == 0x06) { gear_state_data = 2;}
          else if ( (gear_state_raw_data == 0x15) || (gear_state_raw_data == 0x25) ) { gear_state_data = 3;}
          
          std_msgs::msg::UInt8 current_gear_state_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
          current_gear_state_msg.data = gear_state_data;
          current_gear_state_topic_->publish(std::move(current_gear_state_msg)); 
        ...
        { 
          ...
          current_steering_angle_status_msg.steering_tire_angle = (current_steering_tire_angle_status_report); //HH_230314 // rad
          ...
          // autoware_auto_vehicle_msgs::msg::SteeringReport current_steering_angle_status_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
          // current_steering_angle_status_msg.steering_tire_angle = (current_steering_tire_angle_status_report); //HH_230314 // degree
          // current_steering_angle_status_topic_->publish(std::move(current_steering_angle_status_msg));
        ...
        {           
          ...
          current_velocity_status_msg.longitudinal_velocity = (current_velocity_report / 3.6);
        ...
        case 1810:
        {
          ...
          //HH_230816
          autoware_auto_vehicle_msgs::msg::VelocityReport current_velocity_status_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
          current_velocity_status_msg.longitudinal_velocity = current_velocity_report / 3.6; // mps to kph
          current_velocity_status_msg.header.frame_id = "base_link";  //HH_230316
          current_velocity_status_topic_->publish(std::move(current_velocity_status_msg));
        }
      }
