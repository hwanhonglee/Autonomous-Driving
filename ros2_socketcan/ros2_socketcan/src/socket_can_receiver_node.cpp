...
LNI::CallbackReturn SocketCanReceiverNode::on_configure(const lc::State & state)
{
  ...
  //HH_230309
  current_steering_angle_status_topic_ = this->create_publisher<autoware_auto_vehicle_msgs::msg::SteeringReport>("/vehicle/status/steering_status", 1);
  //HH_230314
  current_velocity_status_topic_ = this->create_publisher<autoware_auto_vehicle_msgs::msg::VelocityReport>("/vehicle/status/velocity_status", 1);
  //HH_230401
  current_steering_angle_status_topic_for_rviz_ = this->create_publisher<autoware_auto_vehicle_msgs::msg::SteeringReport>("/vehicle/status/steering_status_for_rviz", 1);
    ...
    while (rclcpp::ok()) {
      ...
      switch(frame_msg.id)
      {
        ...
        case 882: //0x372 //for gear state
        {
          ...
          else if (gear_state_raw_data == 0x15) { gear_state_data = 3;}
