...
LNI::CallbackReturn SocketCanReceiverNode::on_configure(const lc::State & state)
{
  ...
  // HH_240814
  vehicle_status_steering_status_rviz_topic_ = this->create_publisher<autoware_auto_vehicle_msgs::msg::SteeringReport>("/vehicle/status/steering_status_for_rviz", 500); 
  ...
  if (!enable_fd_) {
    ...
    while (rclcpp::ok()) {
      ...
      switch(frame_msg.id)
      {
        ...
        { 
          bool current_EPS_En_ = (frame_msg.data[0] & 0X01);
          short current_EPS_Angle_tmp = (frame_msg.data[3] << 8) | frame_msg.data[2];
          ...
          autoware_auto_vehicle_msgs::msg::SteeringReport vehicle_status_steering_status_for_rivz_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
          ...
          bool current_cruise_status = (frame_msg.data[1] & 0x8A) >> 7;
          bool current_cruise_mode = false;
          u_int8_t vehicle_status_control_mode_report = 4;

          if (current_cruise_status == 1) {
        ...
        case 1809: //0x711 // at every 10 ms // ACC Enable 
        {           
        ...
        case 1810: // 0x712 // current_vehicle_velocity
        {
          int16_t current_velocity_tmp = 0;
          ...
          current_velocity_ = current_velocity_tmp * 0.03125;
          velocity_received_ = true;
          break;
        }

        // HH_240722
        case 1811: // 0x713 // Lateral acceleration and heading rate
        {
          int16_t lateral_accel_raw = (frame_msg.data[1] << 8) | frame_msg.data[0];
          double lateral_accel = 0.01 * (lateral_accel_raw - 1023);
          double dt = 0.001; 
          double lateral_velocity_acc = 0.0;
          
          lateral_velocity_acc += lateral_accel * dt;
          if ( lateral_velocity_acc <= 0.0 ) lateral_velocity_acc = 0.0;
          else if ( lateral_velocity_acc >= 0.00001 ) lateral_velocity_acc -= 0.0002; 

          lateral_velocity_ = lateral_velocity_acc;


          int16_t heading_rate_raw = (frame_msg.data[4] << 8) | frame_msg.data[3];
          heading_rate_ = 0.01 * (heading_rate_raw - 4095);
      ...
      if (velocity_received_ && accel_received_) {
...
{
  autoware_auto_vehicle_msgs::msg::VelocityReport combined_msg;
  combined_msg.header.stamp = this->now();
  combined_msg.header.frame_id = "base_link";
  combined_msg.longitudinal_velocity = current_velocity_ / 3.6; // Convert to m/s
  combined_msg.lateral_velocity = lateral_velocity_;
  combined_msg.heading_rate = heading_rate_;
