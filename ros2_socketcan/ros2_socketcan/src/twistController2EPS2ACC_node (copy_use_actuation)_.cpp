...
public:
  ...
  : Node("twistController2Eps2Acc_node")
  {
    ...
    steering_sub_ = this->create_subscription<tier4_vehicle_msgs::msg::ActuationCommandStamped>(
      "/control/command/actuation_cmd", 1, 
      std::bind(&TwistController2Eps2AccNode::steering_callback, this, std::placeholders::_1));
    
    throttle_sub_ = this->create_subscription<tier4_vehicle_msgs::msg::ActuationCommandStamped>(
      "/control/command/actuation_cmd", 1, 
...
private:
  //HH_230328
  // Subscriber
  rclcpp::Subscription<tier4_vehicle_msgs::msg::ActuationCommandStamped>::SharedPtr steering_sub_;
  rclcpp::Subscription<tier4_vehicle_msgs::msg::ActuationCommandStamped>::SharedPtr throttle_sub_;
  ...
  //HH_230328
  void throttle_callback(const tier4_vehicle_msgs::msg::ActuationCommandStamped::SharedPtr msg)
  {
    throttle_cmd = msg->actuation.accel_cmd ;

    if (throttle_cmd >= 1) throttle_cmd = 1.0;
    else if (throttle_cmd == 0) throttle_cmd = -1.5;
  ...
  {
    steering_cmd =  msg->actuation.steer_cmd * DEG2RAD; // degree
