...
public:
  ...
  : Node("twistController2Eps2Acc_node")
  {
    ...
    steering_sub_ = this->create_subscription<autoware_control_msgs::msg::Control>(
      "/control/command/control_cmd", 1, 
      std::bind(&TwistController2Eps2AccNode::steering_callback, this, std::placeholders::_1));
    
    throttle_sub_ = this->create_subscription<autoware_control_msgs::msg::Control>(
      "/control/command/control_cmd", 1, 
...
private:
  //HH_230328
  // Subscriber
  rclcpp::Subscription<autoware_control_msgs::msg::Control>::SharedPtr steering_sub_;
  rclcpp::Subscription<autoware_control_msgs::msg::Control>::SharedPtr throttle_sub_;
  ...
  //HH_230328
  void throttle_callback(const autoware_control_msgs::msg::Control::SharedPtr msg)
  {
    throttle_cmd = msg->longitudinal.acceleration + 0.3;

    // HH_230907 for edit limit 
    if (throttle_cmd >= 5) throttle_cmd = 5.0;
    // if (throttle_cmd >= 0) throttle_cmd = 0.5;
    else if (throttle_cmd == 0) throttle_cmd = -0.5;
    else if (throttle_cmd <= -5) throttle_cmd = -5.0;
  ...
  void steering_callback(const autoware_control_msgs::msg::Control::SharedPtr msg)
  {
    ...
    //HH_230613
    // if (abs(msg->lateral.steering_tire_angle * RAD2DEG) <= 55.0)
    // { 
    //   steering_cmd =  msg->lateral.steering_tire_angle * RAD2DEG * 10.0;
    // }
    // else if(abs(msg->lateral.steering_tire_angle * RAD2DEG) > 55.0 && abs(msg->lateral.steering_tire_angle * RAD2DEG) <= 60.0)
    // {
    //   steering_cmd =  std::clamp(msg->lateral.steering_tire_angle * RAD2DEG * 2.0 , -438.0, 438.0);
    // }
    // else if(abs(msg->lateral.steering_tire_angle * RAD2DEG) > 60)
    // {
    //   steering_cmd =  std::clamp(msg->lateral.steering_tire_angle * RAD2DEG * 0.8 , -438.0, 438.0);
    // }


    // if (steering_cmd > 5256.0) 
    // {
    //   steering_cmd = 5256.0;
    // }
    // else if (steering_cmd < -5256.0) 
    // {
    //   steering_cmd = -5256.0; 
    // }
    // else 
    ...
    // if (steering_cmd > 7.644542124) 
    // {
    //   steering_cmd = 7.644542124;
    // }
    // else if (steering_cmd < -7.644542124) 
    // {
    //   steering_cmd = -7.644542124; 
    // }
    // else {};
  ...
  // //HH_230328
  // void steering_callback(const tier4_vehicle_msgs::msg::ActuationCommandStamped::SharedPtr msg)
  // {
  //   steering_cmd =  msg->actuation.steer_cmd; // degree

  //   if (steering_cmd > 438) 
  //   {
  //     steering_cmd = 438.0;
  //   }
  //   else if (steering_cmd < -438) 
  //   {
  //     steering_cmd = -438.0;
  //   }
  //   else {};
