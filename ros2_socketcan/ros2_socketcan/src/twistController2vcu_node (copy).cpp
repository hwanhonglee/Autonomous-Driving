...
class TwistController2Pc2Can0Node : public rclcpp::Node
{
public:
  TwistController2Pc2Can0Node()
  : Node("twistController2pc2can0_node")
  {
    ...
    sub_ = this->create_subscription<std_msgs::msg::Bool>(
      ...
      std::bind(&TwistController2Pc2Can0Node::current_cruise_mode_callback, this, std::placeholders::_1));

    // Publish CAN data
    pub_ = this->create_publisher<can_msgs::msg::Frame>("can_tx", 500);
  }
...
private:
  ...
  u_int8_t CAN_alive_count_ = 0;
  
    void current_cruise_mode_callback(const std_msgs::msg::Bool::SharedPtr msg)
  {
    current_cruise_mode_ = msg->data;
    RCLCPP_INFO(this->get_logger(), "Current cruise mode status: %d", current_cruise_mode_);
  ...
  {
    RCLCPP_INFO(this->get_logger(), "Generating CAN message for IONIQ_CtrlVCU");
    ...
    frame_msg.data[0] = (vehicle_type_) & 0x128;
    frame_msg.data[1] = 0;
...
int main(int argc, char **argv)
{
  ...
  rclcpp::spin(std::make_shared<TwistController2Pc2Can0Node>());
  rclcpp::shutdown();
  return 0;
}