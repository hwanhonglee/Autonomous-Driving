...
if git_dir="$(git -C "${root}" rev-parse --git-dir 2>/dev/null)"; then
  ...
  for lfs_extension in png gif; do
    lfs_sample="$(git -C "${root}" ls-files "docs/assets/validation/2026-09-07/control_ab_30kph_and_60kph_readiness_v1/**/*.${lfs_extension}" | head -n 1)"
...
권장 다음 순서 (아래 명령은 표시만 했으며 실행하지 않았습니다)
  ...
  1. 발행된 PNG/GIF까지 보려면 Git LFS를 설치한 뒤:
     git lfs install
     git lfs pull --include='docs/assets/validation/2026-09-07/control_ab_30kph_and_60kph_readiness_v1/**' --exclude=''
