...
def inspect_connection(connection):
    ...
    # HH_260906 - Reject virtual declarations even when SQLite preserves whitespace or comments between their keywords.
    def virtual(sql):
        if not isinstance(sql, str): return False
        uncommented = re.sub(r"/\*.*?\*/|--[^\n]*", " ", sql, flags=re.DOTALL)
        return re.search(r"\bCREATE\s+VIRTUAL\s+TABLE\b", uncommented, flags=re.IGNORECASE) is not None
    require(not any(kind == "view" or virtual(sql) for _, kind, sql in schema), "views/virtual tables are forbidden")
...
def run(args):
    ...
    try:
        ...
        # HH_260906 - A vanished or changed source must produce a failed report, not bypass persistence in this cleanup block.
        postcheck_errors = []
        def postcheck(operation, callback):
            try: return callback()
            except Exception as error:
                postcheck_errors.append({"operation": operation, "error_type": type(error).__name__, "error": str(error)})
                return None
        if connection is not None: postcheck("close_memory_database", connection.close)
        result["archive_post_identity"] = postcheck("archive_stat", lambda: regular(archive_path))
        result["archive_identity_unchanged"] = result["archive_post_identity"] == before
        for name, proof in (("nuplan-v1.1_mini.zip.full-audit.json", full_proof), ("nuplan-v1.1-mini-db-camera0-subset-inspection.json", subset_proof)):
            postcheck("prior_proof:" + name, lambda name=name, proof=proof: metadata(archive_root / "verification" / name, proof["sha256"]))
        result["postcheck_errors"] = postcheck_errors
        if not result["archive_identity_unchanged"] or postcheck_errors: result["status"] = "FAILED_METADATA_DIAGNOSTIC"
