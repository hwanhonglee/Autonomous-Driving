...
python3 - "${output_root}" "${route_path}" "${trial_status}" \
  ...
  "${camera_integrity_status}" "${runtime_load_status}" <<'PY' || handoff_status=$?
import hashlib
import json
from datetime import datetime, timezone
import math
...
if (
    ...
    problems.append("route result is not a completed full-stack PASS")
...
for name, value in (("completion", completion), ("cleanup", cleanup)):
    if value.get("status") != "PASS":
        problems.append(f"CARLA {name} health did not pass")
...
if (( trial_status != 0 || cleanup_status != 0 || coverage_status != 0 ||
      ...
      runtime_load_status != 0 || handoff_status != 0 )); then
  echo "60 kph pilot did not complete every gate: trial=${trial_status} cleanup=${cleanup_status} coverage=${coverage_status} telemetry=${telemetry_cleanup_status} camera=${camera_integrity_status} runtime_load=${runtime_load_status} handoff=${handoff_status}" >&2
  exit 1
fi
pilot_complete=true
