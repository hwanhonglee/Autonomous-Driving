...
usage() {
  cat >&2 <<EOF
Usage: $0 [--full] [--visualize] [--recommended] [--tight-corridor] [--trajectory-stability] [--fp16-heads] [--model-override YAML] [--sensor-mapping YAML] ROUTE_JSON [ros2 launch arguments...]

  default       Minimal Autoware control shell and the lowest runtime load
  --full        Full Autoware shell; RViz stays off unless --visualize is set
...
for argument in "$@"; do
  ...
  if [[ "${recommended}" == "true" ]]; then
    case "${argument}" in
      use_lateral_controller_param_override:=*|lateral_controller_param_path:=*|use_longitudinal_controller_param_override:=*|longitudinal_controller_param_path:=*|controller_stop_offset_m:=*|comfortable_deceleration_mps2:=*|maneuver_lookahead_m:=*|maneuver_exit_lookahead_m:=*|turn_inward_corridor_half_width_m:=*|turn_outward_corridor_half_width_m:=*|left_turn_outward_corridor_half_width_m:=*|right_turn_outward_corridor_half_width_m:=*|route_corridor_entry_distance_m:=*|trajectory_lateral_filter_gain:=*|left_turn_trajectory_lateral_filter_gain:=*|right_turn_trajectory_lateral_filter_gain:=*|trajectory_lateral_filter_activation_threshold_m:=*|trajectory_geometry_smoothing_strength:=*|maximum_lateral_acceleration_mps2:=*|maximum_speed_mps:=*|raw_vehicle_cmd_converter_config:=*)
...
recommended_mpc=""
if [[ "${recommended}" == "true" ]]; then
  fast_mapping="${package_share}/config/sensor_mapping_vad_fast_reliable.yaml"
  ...
  for required in "${fast_mapping}" "${model_override}" "${recommended_mpc}"; do
    if [[ ! -f "${required}" ]]; then
...
echo "VAD fast profile: 6x 640x360 raw cameras at 5 Hz; recommended=${recommended}; tight corridor=${tight_corridor}; trajectory stability=${trajectory_stability}; mixed FP16 heads=${fp16_heads}; sensor mapping=${fast_mapping}" >&2
if [[ "${full}" == "true" ]]; then
  profile_arguments+=(
    "rviz:=${visualize}"
