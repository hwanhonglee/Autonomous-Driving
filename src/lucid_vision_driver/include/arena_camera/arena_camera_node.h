...
#include <chrono>
// HH_260810 - Share handler lifetime safely with the context pre-shutdown callback.
#include <memory>
#include <thread>
...
public:
  ...
  // HH_260810 - Stop Arena callbacks before ROS publisher members are destroyed.
  ~ArenaCameraNode() override;
...
private:
  ...
  // HH_260810 - A weak callback can lock this handler without retaining the ROS node itself.
  std::shared_ptr<ArenaCamerasHandler> m_arena_camera_handler;

  // HH_260810 - Stop the SDK callback before rcl_shutdown invalidates ROS publishers.
  rclcpp::Context::SharedPtr m_context;
  rclcpp::PreShutdownCallbackHandle m_pre_shutdown_callback_handle;
