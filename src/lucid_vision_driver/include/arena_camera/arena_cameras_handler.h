...
// HH_260810 - Serialize SDK stream start/stop across rclcpp shutdown and node destruction.
#include <mutex>
#include <vector>
...
public:
  ...
  // HH_260810 - Safe to call from rclcpp's pre-shutdown callback and again from destructors.
...
//  Arena::IDevice * m_device;
  ...
  // 260604 KYS CAMERA BUG
  Arena::ISystem * m_p_system{nullptr};
  Arena::IDevice * m_device{nullptr};
  ArenaCamera * m_cameras{nullptr};
  // HH_260810 - Make callback gating and StopStream idempotent across both shutdown entry points.
  std::mutex m_shutdown_mutex;
  bool m_streaming{false};
  bool m_callbacks_blocked{false};
