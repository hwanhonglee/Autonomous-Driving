...
: rclcpp::Node("arena_camera_node", node_options.allow_undeclared_parameters(true))
{
  ...
  // HH_260810 - Share only the handler with pre-shutdown; never capture the ROS node lifetime.
  m_arena_camera_handler = std::make_shared<ArenaCamerasHandler>();
  ...
  // HH_260810 - Context::shutdown() invalidates rcl publishers before normal node destruction.
  // Gate and join the Arena callback while the context and every publisher are still valid.
  m_context = this->get_node_base_interface()->get_context();
  const std::weak_ptr<ArenaCamerasHandler> weak_handler = m_arena_camera_handler;
  m_pre_shutdown_callback_handle = m_context->add_pre_shutdown_callback(
    [weak_handler]() noexcept {
      if (const auto handler = weak_handler.lock()) {
        std::cout << "[LUCID SHUTDOWN] rclcpp pre-shutdown callback" << std::endl;
        handler->stop_stream();
      }
    });
...
{
  // HH_260810 - Component unload can happen without global context shutdown.  Remove the callback
  // registration and use the same idempotent gate/StopStream path before releasing the handler.
  if (m_context) {
    try {
      m_context->remove_pre_shutdown_callback(m_pre_shutdown_callback_handle);
    } catch (...) {
      std::cerr << "[LUCID SHUTDOWN] remove_pre_shutdown_callback failed" << std::endl;
    }
  }

  if (m_arena_camera_handler) {
    m_arena_camera_handler->stop_stream();
  }

  // HH_260810 - The handler owns the SDK callback bound to this node.  It is declared before the
  // publishers, so implicit member destruction would otherwise stop the stream only after those
  // publishers were gone.  Reset it while every callback dependency is still alive.
  m_arena_camera_handler.reset();
}
