...
// HH_260810 - Added an explicit worker-state flag so lifecycle cleanup can stop CAN reception.
#include <atomic>
#include <memory>
#include <thread>
#include <string>
...
public:
  ...
  // HH_260810 - Prevent a joinable receive thread from triggering std::terminate at destruction.
  /// \brief Stop and join the receive thread before member destruction.
...
private:
  ...
  // HH_260810 - Share one deterministic stop-and-join path across cleanup and shutdown callbacks.
  ...
  // HH_260810 - Stop the worker independently of the global ROS context to avoid lifecycle hangs.
  std::atomic_bool receiver_running_{false};
