...
// HH_260810 - Reuse the guarded stop path when the lifecycle node is destroyed by Ctrl+C.
...
{
  // HH_260810 - End and join the worker explicitly so Ctrl+C cannot destroy a joinable thread.
...
LNI::CallbackReturn SocketCanReceiverNode::on_configure(const lc::State & state)
{
  ...
  // HH_260810 - Mark the worker active before construction so every exit path can stop it safely.
...
LNI::CallbackReturn SocketCanReceiverNode::on_cleanup(const lc::State & state)
{
  ...
  // HH_260810 - Join reception before publishers are reset to prevent a publish/reset race.
  stop_receiver_thread();
...
{
  (void)state;
  // HH_260810 - Lifecycle shutdown must not leave the receive worker joinable.
  stop_receiver_thread();
...
void SocketCanReceiverNode::receive()
{
  ...
  if (!enable_fd_) {
    ...
    // HH_260810 - Honor both ROS shutdown and lifecycle cleanup when leaving the receive loop.
    while (rclcpp::ok() && receiver_running_.load()) {
      ...
      try {
        ...
        // HH_260810 - An idle SocketCAN poll timeout is expected and does not require a warning.
        continue;
      } catch (const std::exception & ex) {
      ...
      switch(frame_msg.id)
      {
        ...
        case 882: //0x372 //for gear state
        {
          ...
          // HH_260811 - Audit note: these dual raw gear encodings predate v1.1 and require
          // validation against a recorded vehicle CAN trace before being treated as canonical.
          if (gear_state_raw_data == 0x20 || gear_state_raw_data == 0x10) { gear_state_data = 22;} // P
          else if (gear_state_raw_data == 0x27 || gear_state_raw_data == 0x17) { gear_state_data = 20;} // R
          else if (gear_state_raw_data == 0x26 || gear_state_raw_data == 0x16) { gear_state_data = 1;} // N
          else if (gear_state_raw_data == 0x25 || gear_state_raw_data == 0x15) { gear_state_data = 2;} // D
    ...
    // HH_260810 - Apply the same deterministic stop condition to the CAN-FD receive loop.
    while (rclcpp::ok() && receiver_running_.load()) {
      if (this->get_current_state().id() != State::PRIMARY_STATE_ACTIVE) {
        std::this_thread::sleep_for(100ms);
        continue;
      ...
      try {
        ...
        // HH_260810 - An idle SocketCAN FD poll timeout is expected and does not require a warning.
        continue;
      } catch (const std::exception & ex) {
        RCLCPP_WARN_THROTTLE(
          this->get_logger(), *this->get_clock(), 1000,
