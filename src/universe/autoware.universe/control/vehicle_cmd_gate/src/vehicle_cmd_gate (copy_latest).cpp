...
: Node("vehicle_cmd_gate", node_options), is_engaged_(false), updater_(this)
{
  ...
  // HH_240219
  // Initialize goal_marker_poses_
  goal_marker_poses_.clear(); // Clear the vector
  ...
  // HH_240216 
  // Initialize the publisher for goal reached status 
  pub_goal_reached_ = this->create_publisher<std_msgs::msg::Bool>("goal_reached", 10); 
  // HH_240304
  // pub_routing_mode_pub_ = this->create_publisher<std_msgs::msg::Bool>("goal_reached", 10); 
  ...
  // HH_240216
  // Subscriber for current vehicle pose 
  sub_current_pose_ = this->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>( 
    "/sensing/gnss/pose_with_covariance", 10, 
    std::bind(&VehicleCmdGate::on_current_pose, this, std::placeholders::_1)); 
  // Subscribers for goal pose markers 
  sub_goal_marker_ = this->create_subscription<visualization_msgs::msg::MarkerArray>( 
    "/rviz2/automatic_goal/markers", 10, 
    std::bind(&VehicleCmdGate::on_goal_marker, this, std::placeholders::_1)); 
    // Subscriber for goal pose from mission planning 
    sub_goal_pose_ = this->create_subscription<geometry_msgs::msg::PoseStamped>( 
      "/planning/mission_planning/echo_back_goal_pose", 10, 
    std::bind(&VehicleCmdGate::on_goal_pose, this, std::placeholders::_1)); 
  // HH_240216
...
// HH_240216
// for subscribe
void VehicleCmdGate::on_current_pose(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
{ 
  current_pose_ = msg->pose.pose; 
  current_pose_received_ = true;
}

void VehicleCmdGate::on_goal_marker(const visualization_msgs::msg::MarkerArray::SharedPtr msg)
{
    // Clear previous goal markers
    goal_marker_poses_.clear();

    // Process all goal markers
    for (const auto& marker : msg->markers) {
        goal_marker_poses_.push_back(marker.pose);
    }

    // Set the flag to indicate that at least one goal marker has been received
    if (!goal_marker_poses_.empty()) {
        loop_goal_pose_received_ = true;
    }
}

void VehicleCmdGate::on_goal_pose(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
  goal_pose_ = msg->pose;
...
// bool VehicleCmdGate::is_goal_reached() const
// { 
//   if (!goal_pose_received_ || !current_pose_received_) { 
//     return false; // 목표 위치 또는 현재 위치 정보가 받아지지 않았을 경우 
//   }
//   double distance_threshold = 1.0; // 목표 위치와의 거리 임계값 (조정 가능)
//   double dx_goal_to_current = goal_pose_.position.x - current_pose_.position.x; 
//   double dy_goal_to_current = goal_pose_.position.y - current_pose_.position.y; 
//   double distance_goal_to_current = std::sqrt(dx_goal_to_current * dx_goal_to_current + dy_goal_to_current * dy_goal_to_current);
  
//   double dx_loop_goal_to_current = goal_marker_poses_.position.x - current_pose_.position.x; 
//   double dy_loop_goal_to_current = goal_marker_poses_.position.y - current_pose_.position.y; 
//   double distance_loop_goal_to_current = std::sqrt(dx_loop_goal_to_current * dx_loop_goal_to_current + dy_loop_goal_to_current * dy_loop_goal_to_current);
  
//   return ( (distance_goal_to_current < distance_threshold) || (distance_loop_goal_to_current < distance_threshold) );
// }
// // HH_240216
// HH_240219
bool VehicleCmdGate::is_goal_reached() const {
    ...
    // 루프 목표 위치들에 대해 각각 거리를 계산하여 확인
    for (const auto& goal_marker_pose : goal_marker_poses_) {
        double dx_loop_goal_to_current = goal_marker_pose.position.x - current_pose_.position.x;
        double dy_loop_goal_to_current = goal_marker_pose.position.y - current_pose_.position.y;
        double distance_loop_goal_to_current = std::sqrt(dx_loop_goal_to_current * dx_loop_goal_to_current + dy_loop_goal_to_current * dy_loop_goal_to_current);

        if (distance_loop_goal_to_current < distance_threshold) {
            return true; // 루프 목표 위치에 도달한 경우
        }
    }

    // 모든 루프 목표 위치들에 대해 도달하지 못한 경우 목표 위치에 대해 확인
    return (distance_goal_to_current < distance_threshold);
}
// HH_240219
...
void VehicleCmdGate::onTimer()
{
  ...
  // HH_240304
  autoware_ad_api::routing::RouteState route_mode_msg;

  // Check if the vehicle has reached the goal pose
  if ( (is_goal_reached()) && (current_gate_mode_.data == GateMode::AUTO) ) {
    current_operation_mode_.mode = OperationModeState::STOP;
    is_engaged_ = false;
    
    // Publish the goal reached status
    goal_reached_msg.data = true; // Assuming goal is reached
    pub_goal_reached_->publish(goal_reached_msg);
    
    route_mode_msg.data = 3;
    
  } 
  else if ( (goal_reached_msg.data)  && ( (goal_pose_received_) || (loop_goal_pose_received_) ) && (current_operation_mode_.mode == OperationModeState::STOP) ) {
    current_operation_mode_.mode = OperationModeState::AUTONOMOUS;
    is_engaged_ = true;
  
    // Publish the goal reached status
    goal_reached_msg.data = false; // Assuming goal is reached
    pub_goal_reached_->publish(goal_reached_msg);
  }

  // Publish gate mode, engage, and operation mode status
