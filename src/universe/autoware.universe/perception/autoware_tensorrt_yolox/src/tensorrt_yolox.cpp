...
bool TrtYoloX::feedforward(const std::vector<cv::Mat> & images, ObjectArrays & objects)
{
  ...
  for (size_t i = 0; i < batch_size; ++i) {
    ...
    for (size_t j = 0; j < num_detection; ++j) {
      ...
      // HH_260811 - Clamp both corners before publishing ROIs so every fusion consumer
      // receives a positive box that is fully contained in the source image.
      const auto left = std::clamp(static_cast<int32_t>(x1), 0, images[i].cols);
      const auto top = std::clamp(static_cast<int32_t>(y1), 0, images[i].rows);
      const auto right = std::clamp(static_cast<int32_t>(x2), left, images[i].cols);
      const auto bottom = std::clamp(static_cast<int32_t>(y2), top, images[i].rows);
      object.x_offset = left;
      object.y_offset = top;
