...
: Node("accel_brake_map_calibrator", node_options)
{
...

  {
    ...
    RCLCPP_DEBUG(get_logger(), "Attempting to read accel map from: %s", csv_path_accel_map.c_str());
    RCLCPP_DEBUG(get_logger(), "Attempting to read brake map from: %s", csv_path_brake_map.c_str());
  ...
  actuation_cmd_sub_ = create_subscription<ActuationCommandStamped>(
    ...
    std::bind(&AccelBrakeMapCalibrator::callbackActuationCmd, this, _1));
...
void AccelBrakeMapCalibrator::callbackActuationCmd(
...
void AccelBrakeMapCalibrator::takeConsistencyOfAccelMap()
{
  ...
  for (std::size_t ped_idx = 0; ped_idx < update_accel_map_value_.size() - 1; ped_idx++) {
    for (std::size_t vel_idx = 0; ped_idx < update_accel_map_value_.at(0).size() - 1; ped_idx++) {
...
void AccelBrakeMapCalibrator::updateTotalMapOffset(const double measured_acc, const double map_acc)
{
  ...
  /* update map */
  for (std::size_t ped_idx = 0; ped_idx < update_accel_map_value_.size() - 1; ped_idx++) {
    for (std::size_t vel_idx = 0; vel_idx < update_accel_map_value_.at(0).size() - 1; ped_idx++) {
