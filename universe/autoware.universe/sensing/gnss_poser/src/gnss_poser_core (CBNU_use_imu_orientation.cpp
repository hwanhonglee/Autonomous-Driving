...
: rclcpp::Node("gnss_poser", node_options),
  ...
  // HH_240625 // use imu, no ins 
  use_imu_orientation_(declare_parameter<bool>("use_imu_orientation", true)), // must be initialize "true or false"  
  msg_imu_orientation_(std::make_shared<geometry_msgs::msg::QuaternionStamped>()),
  ...
  gnss_pose_pub_method(declare_parameter<int>("gnss_pose_pub_method"))
{
  ...
  // Subscribe
  nav_sat_fix_sub_ = create_subscription<sensor_msgs::msg::NavSatFix>("fix", rclcpp::QoS{1}, std::bind(&GNSSPoser::callbackNavSatFix, this, std::placeholders::_1));
  imu_orientation_sub_ = create_subscription<sensor_msgs::msg::Imu>("imu/data", rclcpp::QoS{1}, std::bind(&GNSSPoser::callbackImu, this, std::placeholders::_1));
  ...
  pose_cov_pub_ = create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>("gnss_pose_cov", rclcpp::QoS{1}); //HH_230413 // GNSS Pose 
  fixed_pub_ = create_publisher<tier4_debug_msgs::msg::BoolStamped>("gnss_fixed", rclcpp::QoS{1});

  // HH_240625
  autoware_orientation_pub_ = create_publisher<autoware_sensing_msgs::msg::GnssInsOrientationStamped>("autoware_orientation", rclcpp::QoS{1});
} 
...
void GNSSPoser::callbackNavSatFix(const sensor_msgs::msg::NavSatFix::ConstSharedPtr nav_sat_fix_msg_ptr)
{
  ...
  if (!received_map_projector_info_) {RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), std::chrono::milliseconds(1000).count(),
  ...
  // HH_240625 
  // calc imu orientation
  geometry_msgs::msg::Quaternion orientation;
  if (use_imu_orientation_) {
    orientation = msg_imu_orientation_->quaternion;
  } else {
  ...
  // HH_240625
  // Publish GnssInsOrientationStamped
  auto autoware_orientation_msg = autoware_sensing_msgs::msg::GnssInsOrientationStamped();
  autoware_orientation_msg.header.stamp = this->now();
  autoware_orientation_msg.header.frame_id = map_frame_;
  autoware_orientation_msg.orientation.orientation = orientation;
  // Assuming RMSE values need to be filled
  autoware_orientation_msg.orientation.rmse_rotation_x = 0.0; // Replace with actual value if available
  autoware_orientation_msg.orientation.rmse_rotation_y = 0.0; // Replace with actual value if available
  autoware_orientation_msg.orientation.rmse_rotation_z = 0.0; // Replace with actual value if available
  autoware_orientation_pub_->publish(autoware_orientation_msg);
  ...
  // HH_240321 // lat_long // sensing/gnss/fix data type
  // gnss_base_pose_cov_msg.pose.pose.position.x = gnss_base_pose_msg.pose.position.x - 36.62438409;
  // gnss_base_pose_cov_msg.pose.pose.position.y = gnss_base_pose_msg.pose.position.y - 127.45583925;
  
  // HH_240327 // set, sensing/gnss/pose_with_covariance data 
  gnss_base_pose_cov_msg.pose.pose.position.x = gnss_base_pose_msg.pose.position.x - 61928.920906187035;
  gnss_base_pose_cov_msg.pose.pose.position.y = gnss_base_pose_msg.pose.position.y - 54315.49627994234;
...
{
  msg_imu_orientation_->header = msg->header;
  msg_imu_orientation_->quaternion = msg->orientation;
}
