  ...
  canceled_{false}
{
  ...
  if (options.use_intra_process_comms()) {
    ...
    info_pub_ = create_publisher<sensor_msgs::msg::CameraInfo>("camera_info", 1);
  } else {
    //HH_231123 // HH_240819
    auto qos = rclcpp::SensorDataQoS();
    camera_transport_pub_ = image_transport::create_camera_publisher(this, "image_raw", qos.get_rmw_qos_profile());
    // camera_transport_pub_ = image_transport::create_camera_publisher(this, "image_raw", rclcpp::SensorDataQoS());
  }
...
// {
//     RCLCPP_INFO(this->get_logger(), "Applying parameters...");

//     output_encoding_ = sensor_msgs::image_encodings::BGR8;  // 여기서 인코딩 형식 설정
...
//     RCLCPP_INFO(this->get_logger(), "Requested pixel format: %s", pixel_format.c_str());
//     if (!requestPixelFormat(pixel_format)) {
//         RCLCPP_WARN(this->get_logger(), "Requested pixel format not supported: %s", pixel_format.c_str());
//     }

//     // Image size parameters
//     auto image_size = parameters_.getImageSize();
//     RCLCPP_INFO(this->get_logger(), "Requested image size: %ldx%ld", image_size[0], image_size[1]);
//     if (!requestImageSize(image_size)) {
//         RCLCPP_WARN(this->get_logger(), "Requested image size not supported: %ldx%ld", image_size[0], image_size[1]);
//     }

//     // Control parameters 설정
...
sensor_msgs::msg::Image::UniquePtr V4L2Camera::convert(sensor_msgs::msg::Image const & img) const
{
  ...
  cv::Mat yuv_image = cv_bridge::toCvCopy(img, img.encoding)->image;
  cv::Mat rgb_image;
  
  // Convert YUV422 to RGB
  cv::cvtColor(yuv_image, rgb_image, cv::COLOR_YUV2RGB_YUY2);
  
  // Create a new sensor_msgs::msg::Image to hold the RGB image
  auto outImg = std::make_unique<sensor_msgs::msg::Image>();
  cv_bridge::CvImage(std_msgs::msg::Header(), "bgr8", rgb_image).toImageMsg(*outImg);
